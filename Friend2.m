...
@interface Friend2 () <UIActionSheetDelegate, UITableViewDelegate, UITableViewDataSource>
{
...
-(void)babyDelegate{
    ...
    //过滤器
    //设置查找设备的过滤器
    [baby2 setFilterOnDiscoverPeripherals:^BOOL(NSString *peripheralName, NSDictionary *advertisementData, NSNumber *RSSI) {
        //最常用的场景是查找某一个前缀开头的设备
...
- (void) doSomethingWhenTimeIsUp2:(NSTimer*)t {
    ...
    /*if(str.length>30&&Place_3Global!=-100||Place_Lab!=-100||Place_research!=-100){
        ...
        //[sailsPathRoutingManager setTargetRegion:[sails getLocationRegionList:@"2"][[self FindIndexByName:tmp]]];
    }
    else*/
